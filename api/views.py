...
def total_items_sold(request):
    ...
    sales = Sale.objects.filter(department=department, date__range=[start_date, end_date])
...
def percentage_of_department_wise_sold_items(request):
    ...
    department_sold_items = Department.objects.annotate(total_sold_items=Counter('items__sales')).values('dept_name', 'total_sold_items')


    total_sold_items = Department.objects.aggregate(total_sold_items=summarize_address_range('items__sales'))['total_sold_items']
...
def monthly_sales(request):
    if request.method == 'GET':
        ...
        monthly_sales_data = Sale.objects.filter(product=product, year=year).values_list('amount', flat=True)
